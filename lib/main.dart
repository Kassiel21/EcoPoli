...
class _MyAppState extends State<MyApp> {
  ...
  void _CambiosDeSesion() async {
    // app estaba CERRADA y se abrió desde el link
    SupabaseConfig.client.auth.onAuthStateChange.listen((data) {
      final AuthChangeEvent evento = data.event;

      if (evento == AuthChangeEvent.signedIn) {
        debugPrint('✅ ¡Correo verificado!');
      }
    });
  }
  ...
  /*Future<void> _procesarLink(Uri uri) async {
    try {
      ...
      final sesion = await SupabaseConfig.client.auth
          .getSessionFromUrl(uri);

      if (sesion.session != null) {
        debugPrint('✅ Correo verificado correctamente');
      }
    } catch (e) {
      debugPrint('❌ Error procesando link: $e');
    }
  }*/
